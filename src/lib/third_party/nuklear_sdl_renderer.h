 ...
 *
 * Todo:
 * Make global context non-global
 *
 */
/*
...
NK_INTERN float nk_sdl_window_to_nk_space_x(float x) {
    return x * sdl.win_scale_x / sdl.render_scale_x;
}

NK_INTERN int nk_sdl_window_to_nk_space_x(int x) {
    return x * sdl.win_scale_x / sdl.render_scale_x;
}

NK_INTERN float nk_sdl_window_to_nk_space_y(float y) {
    return y * sdl.win_scale_y / sdl.render_scale_y;
}

NK_INTERN int nk_sdl_window_to_nk_space_y(int y) {
    return y * sdl.win_scale_y / sdl.render_scale_y;
}
...
NK_INTERN void nk_sdl_device_upload_atlas(const void* image, int width, int height) {
    ...
    SDL_Texture* g_SDLFontTexture = SDL_CreateTexture(sdl.renderer, SDL_PIXELFORMAT_ARGB8888,
...
NK_API void nk_sdl_render(enum nk_anti_aliasing AA) {
    ...
    struct nk_sdl_device* dev = &sdl.ogl;
    float scaleX, scaleY;
    SDL_GetRenderScale(sdl.renderer, &scaleX, &scaleY);
    SDL_SetRenderScale(sdl.renderer, sdl.render_scale_x, sdl.render_scale_y);
    {
        SDL_Rect saved_clip;
    ...
    SDL_SetRenderScale(sdl.renderer, scaleX, scaleY);
}
...
NK_API struct nk_context* nk_sdl_init(SDL_Window* win, SDL_Renderer* renderer, float scale = 1.0f) {
#ifndef NK_SDL_CLAMP_CLIP_RECT
    /* warn for cases where NK_SDL_CLAMP_CLIP_RECT should have been set but isn't */
    const char* name = SDL_GetRendererName(renderer);
    int runtimeVer = SDL_GetVersion();
    if (strncmp("metal", name, 5) == 0 && runtimeVer < SDL_VERSIONNUM(2, 0, 22)) {
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                    "renderer is using Metal API but runtime SDL version %d.%d.%d is older than "
                    "compiled version %d.%d.%d, "
                    "which may cause issues with rendering",
                    SDL_VERSIONNUM_MAJOR(runtimeVer), SDL_VERSIONNUM_MINOR(runtimeVer),
                    SDL_VERSIONNUM_MICRO(runtimeVer), SDL_MAJOR_VERSION, SDL_MINOR_VERSION,
                    SDL_MICRO_VERSION);
    }
#endif
    sdl.win = win;
    sdl.renderer = renderer;
    sdl.win_scale_x = sdl.win_scale_y = 1.0;
    sdl.render_scale_x = sdl.render_scale_y = scale;
    ...
    switch (evt->type) {
        ...
        case SDL_EVENT_MOUSE_MOTION:
            if (ctx->input.mouse.grabbed) {
                ...
                nk_input_motion(ctx, x + nk_sdl_window_to_nk_space_x(evt->motion.xrel),
                                y + nk_sdl_window_to_nk_space_y(evt->motion.yrel));
            } else
                nk_input_motion(ctx, nk_sdl_window_to_nk_space_x(evt->motion.x),
                                nk_sdl_window_to_nk_space_y(evt->motion.y));
            return 1;
        ...
        case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED: {
            ...
            int ww = 0;
            int wh = 0;
            SDL_GetWindowSize(sdl.win, &ww, &wh);
            sdl.win_scale_x = (float)w / ww;
            sdl.win_scale_y = (float)h / wh;
        }
    }
    return 0;
}
